...
public class JdWaybillApiHelper {
    private static Logger log = LoggerFactory.getLogger(JdWaybillApiHelper.class);
     ...
     * @return
     */
    public static ApiResultVo<WaybillCodeResponse> getWaybillCode(String appKey, String appSecret, String accessToken, WaybillReceiveRequest request) {
        // 拉取接口
        String result = getWaybillCode(appKey, appSecret, accessToken, JSONObject.toJSONString(request));

        if (!StringUtils.hasText(result)) return ApiResultVo.error(ApiResultVoEnum.ApiException, "接口请求错误");

        JSONObject jsonObject = JSONObject.parseObject(result);

        if (jsonObject.getInteger("statusCode") != 0)
            return ApiResultVo.error(ApiResultVoEnum.ApiException, jsonObject.getString("statusMessage"));


        return ApiResultVo.success(jsonObject.getObject("data",WaybillCodeResponse.class));
    }

    protected static String getWaybillCode(String appKey,String appSecret,String accessToken,String jsonParams){
        ...
        params.put("method", "jingdong.ldop.alpha.waybill.receive");
        params.put("v", "2.0");
        params.put("timestamp", DateUtil.getCurrentDateTime());
        try {
            String orderNo="296716636166";
            Map<String, Object> pmap = new TreeMap();
            pmap.put("content", jsonParams);
...
//
//
//    public static ApiResultVo<WaybillAccount> getWaybillProvider(String appKey, String appSecret, String accessToken, String vendorCode) {
//        // 拉取接口
//        String result = pullWaybillProvider(appKey, appSecret, accessToken, vendorCode);
//
//        if (!StringUtils.hasText(result)) return ApiResultVo.error(ApiResultVoEnum.ApiException, "接口请求错误");
//
//
...
//    }
//
//    protected static String pullWaybillProvider(String appKey,String appSecret,String accessToken,String vendorCode){
//        String url="https://api.jd.com/routerjson";
//
//        Map<String, String> params = new HashMap<>();
//        params.put("app_key", appKey);
